...
public class JsonExpansions_EngineDeserializer {
    ...
    private void continueFromSerial(SerialRoot root, ExpansionIdentity identity, Knowledge knowledge) {
        if (root.dynamic != null) {
            for (Entry<String, SerialDynamic> entry : root.dynamic.entrySet()) {
                ...
                elements.add(
                        new Dynamic(dynamicSheetHash(entry.getKey()), providers.getSheetCommander(), sheetIndexes));
            }
        }
        if (root.list != null) {
        ...
        if (root.set != null) {
            for (Entry<String, SerialSet> entry : root.set.entrySet()) {
                elements.add(new Junction(entry.getKey(), providers.getCondition(), asList(entry.getValue().yes),
                        asList(entry.getValue().no)));
            }
        }
        if (root.event != null) {
            for (Entry<String, SerialEvent> entry : root.event.entrySet()) {
                elements.add(new Event(entry.getKey(), providers.getSoundRelay(), asList(entry.getValue().path),
    ...
    private void deserializeMachine(Entry<String, SerialMachine> entry) {
        ...
        if (doubleMachine) {
            // If a machine doesn't have a list defined, copy the list from the other
            // machine
        ...
        for (int m = 0; m < (doubleMachine ? 2 : 1); m++) {
            MachineType type = !doubleMachine ? MachineType.NORMAL
                    : (m == 0 ? MachineType.INDOOR : MachineType.OUTDOOR);
    ...
    private void deserializeMachineFromLists(Entry<String, SerialMachine> entry, SerialMachine serial,
        ...
        if (serialEventList != null) {
            serialEventList.forEach(x -> {
                events.add(new TimedEvent(providers.getEvent(), x));
            });
        }
        ...
        if (serialStreamList != null) {
            for (SerialMachineStream sms : serialStreamList) {
                streamList.add(new StreamInformation(machineName, providers.getMachine(), providers.getReferenceTime(),
                        providers.getSoundRelay(), sms.path, sms.vol, sms.pitch, serial.delay_fadein,
                        serial.delay_fadeout, serial.fadein, serial.fadeout, sms.looping, sms.pause, sms.underwater));
        ...
        if (!events.isEmpty()) {
            tie = new TimedEventInformation(machineName, providers.getMachine(), providers.getReferenceTime(), events,
    ...
    private List<SerialMachineEvent>[] getSerialMachineEventList(Entry<String, SerialMachine> entry,
            SerialMachine serial) {
        List<SerialMachineEvent>[] eventList = new ArrayList[2];

        if (serial.event_pair != null) {
            if (serial.event != null) {
                Matmos.LOGGER.warn(
                        "Machine " + entry.getKey() + " has both 'event' and 'event_pair' defined. Ignoring 'event'.");
            }
    ...
    private List<SerialMachineStream>[] getSerialMachineStreamLists(Entry<String, SerialMachine> entry,
            SerialMachine serial) {
        ...
        if (serial.stream_pair != null) {
            if (serial.stream != null) {
                Matmos.LOGGER.warn("Machine " + entry.getKey()
                        + " has both 'stream' and 'stream_pair' defined. Ignoring 'stream'.");
            }
