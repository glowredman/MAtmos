...
public class IDDealiaser {
    ...
    private int getItemID(Item item) {
        if(item instanceof ItemBlock) {
            return Block.getIdFromBlock(((ItemBlock)item).getBlock());
        } else {
    ...
    private int getIDFromName(String name) {
        if(Block.REGISTRY.containsKey(new ResourceLocation(name))) {
            return Block.getIdFromBlock(Block.getBlockFromName(name));
        } else {
            return Item.getIdFromItem((Item)Item.REGISTRY.getObject(new ResourceLocation(name)));
        }
    }
