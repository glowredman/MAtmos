...
public class ScanRaycast extends Scan {
    ...
    public static RayTraceResult rayTraceNonSolid(Vec3d start, Vec3d end) {
        ...
        while (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK && ScanAir
                .isTransparentToSound(w.getBlockState(result.getBlockPos()), w, new BlockPos(result.hitVec), true)) {
    ...
    private void castRay(Vec3d dir) {
        ...
        if (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK) { // ray hit a solid block
            ...
            if (dir.y > 0) { // and it's because we hit the sky, probably
                ...
                BlockPos rayEndBlockPos = new BlockPos(MathHelper.floor(rayEnd.x), MathHelper.floor(rayEnd.y),
                        MathHelper.floor(rayEnd.z));
